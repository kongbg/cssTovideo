<template>
    ...
    <Table border align="center" :tableData="tableInfo.tableData" :columns="columns" :loading="tableInfo.loading"
        ...
        <template #action="{ row }">
            ...
            <el-button size="small" type="primary" v-if="row.fieldJson==null" @click="tableBtnFn('createMode', row)" text>建立模型</el-button>
            <el-button size="small" type="primary" v-if="row.fieldJson" @click="tableBtnFn('editMode', row)" text>编辑模型</el-button>
...
export default defineComponent({ name: "Mode" });
</script>
...
import { getData as getList, create, update } from '@/api/mode.js'
