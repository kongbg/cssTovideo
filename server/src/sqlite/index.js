    ...
    this.db.get(checkTableExistsQuery, [], (err, table) => {
      ...
      if (!table) {
        ...
        const updates = columns.map(column => {
          // 检查列是否已存在
          const checkColumnExistsQuery = `PRAGMA table_info(${tableName});`;
          return new Promise((resolve, reject) => {
            this.db.get(checkColumnExistsQuery, [], (err, info) => {
              if (err) {
                reject(err);
              } else {
                let rowKeys = Object.keys(info)
                if (!rowKeys.includes(column.name)) { // 列不存
                  // 添加列
                  let addStr = `${column.name} ${column.type} ${column.default ? 'DEFAULT '+column.default : ''};`
                  this.db.run(`ALTER TABLE ${tableName} ADD COLUMN ${addStr}`, (err) => {
                    if (err) {
                      console.error(err.message);
                      reject(err)
                    } else {
                      console.log(`列：${column.name} 新增成功`);
                      resolve()
                    }
                  });
                }
